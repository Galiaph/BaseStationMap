...
export default {
  ...
  methods: {
      ...
      select_st: function (event) {
        switch (event.st) {
          case '2g':
            ...
            //   if (element.bs_2g === 1) {
            //     element.show = this.operatorGroup[event.id-1].standart2G === 1 ? true : false
            //   }
            // })
            break
          case '3g':
            ...
            // this.baseStationsGroup.forEach(element => {
            //   if (element.bs_3g === 1) {
            //     element.show = this.operatorGroup[event.id-1].standart3G === 1 ? true : false
            //   }
            // })
            break
          case '4g':
      ...
      getBaseStationById: async function (id) {
        ...
        this.baseStationsGroup.forEach(el => el.show = true)
      },
      ...
      delBaseStationById: function (id) {
        this.baseStationsGroup = this.baseStationsGroup.filter(item => {
          if (item.id === id) {
            return false
          }
